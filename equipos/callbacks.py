...
def lov_team(lov_majorLeague=None, lov_season=None):
    ...
    df = f.filter_df(dataset_name=object_specs["lov_team"]["dataset_name"], filter_cols=filter_cols)
...
def fig_winPercentage(lov_majorLeague=None, lov_season=None, lov_team=None):
    ...
    df = f.filter_df(
        dataset_name=object_specs["fig_winPercentage"]["dataset_name"], filter_cols=filter_cols
    )
...
def fig_runDifferential(lov_majorLeague=None, lov_season=None, lov_team=None):
    filter_cols = {
        "majorLeagueId": lov_majorLeague,
        "seasonId": lov_season,
        "teamId": lov_team,
    ...
    df = f.filter_df(
        dataset_name=object_specs["fig_runDifferential"]["dataset_name"], filter_cols=filter_cols
    )
    obj = f.create_px_figure(
        df=df,
